...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!request_id || !stack_trace_base64 
            || typeof request_id !== 'string' || typeof stack_trace_base64 !== 'string') {
            return res.status(400).json({ error: 'request_id and base64Data are required.' });
        }

        // Obtain an obfuscator instance
        const obfuscator: Obfuscator = getObfuscator();

        return await obfuscator.processStacktrace(req, res, next, request_id, stack_trace_base64);
    } catch (error) {
