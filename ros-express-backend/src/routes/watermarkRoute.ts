...
const upload = multer({
    dest: process.env.ROS_UPLOADS_WATERMARK_STORAGE || "uploads-watermark/",
});
...
router.post(
    ...
    async (req: Request, res: Response, next: NextFunction): Promise<any> => {
        ...
        if (!files || !files.jar) {
            const err = new Error(
                `You must provide a 'jar' file field (types: jar).`
            );
        ...
        try {
            ...
            const data = await obfuscator.extractWatermark(
                req,
                res,
                next,
                jarFile.path
            ...
            const err = new Error(
                "Error calling watermark. Please check the server logs."
            );
