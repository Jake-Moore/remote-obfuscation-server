...
export function getObfuscatorPath(): string | null {
    ...
    const obfuscatorPath =
        process.env[getObfuscatorPathEnvVar()] || getDefaultObfuscatorPath();
    if (
        !fs.existsSync(obfuscatorPath) ||
        !fs.lstatSync(obfuscatorPath).isFile()
    ) {
        return null;
    }
...
export function getObfuscatorType(): string {
    const obfType: string | null =
        process.env[getObfuscatorTypeEnvVar()] || null;
    if (obfType === null) {
