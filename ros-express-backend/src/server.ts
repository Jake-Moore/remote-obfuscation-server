...
if (requiredOrgs.length === 0) {
    ...
    console.log(
        colors.yellow(
            "Warning: No required organizations set. All requests will be denied."
        )
    );
    console.log(
        colors.yellow(
            "Please set the ROS_REQUIRED_ORGS environment variable to a comma-delimited list of required organization names."
        )
    );
    console.log(
        colors.yellow(
