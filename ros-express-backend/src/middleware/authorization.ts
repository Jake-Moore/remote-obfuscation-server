...
export default async function validateAuthorization(req: Request, res: Response, next: NextFunction) {
    ...
    if (!req.headers.authorization) {
        ...
        console.log(colors.red(`[AUTH] Request denied from ${req.ip} - ${err.message}`));
        return next(err);
    }
    ...
    if (!tokenRegex.test(token)) {
        ...
        (err as any).status = 401;
        console.log(colors.red(`[AUTH] Request denied from ${req.ip} - ${err.message} - '${token}'`));
        return next(err);
    }
    ...
    try {
        ...
        if (!Array.isArray(response.data)) {
            ...
            (err as any).status = 401;
            console.log(colors.red(`[AUTH] Request denied from ${req.ip} - ${err.message} - Malformed response data`));
            return next(err);
        }

        // Compose an array of this user's orgs from the 'login' field in each object of the array
        const organizations = response.data.map((org: { login: string }) => org.login || '').filter(org => org.length > 0);
        ...
        console.log(colors.green(`[AUTH] Request authorized from ${req.ip} - ${userEmail}`));
        return next();
    } catch (error) {
        console.log(colors.red(`[AUTH] Request denied from ${req.ip} - ${(error as Error).message}`));
...
export async function getUserEmail(token: string): Promise<string> {
    ...
    const primary = response.data.find((email: { primary: boolean, email: string }) => email.primary);
